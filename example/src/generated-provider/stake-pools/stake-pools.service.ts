...
export class StakePoolsService {
  ...
  async postMaintenanceAction(input: PostStakePoolsMaintenanceActionsInput): Promise<any> {
        ...
        try {
          const response = await this.apiClient.stakePools.postMaintenanceAction(input as PostMaintenanceActionPayload, extraConfig);
          return response.data;
        } catch (error: any) {
  ...
  async quitStakePool(walletId: string, input: DeleteStakePoolsWalletsWalletIdInput): Promise<any> {
        ...
        try {
          const response = await this.apiClient.stakePools.quitStakePool({ walletId }, input as QuitStakePoolPayload, extraConfig);
          return response.data;
        } catch (error: any) {
            if (error.response?.status === 400) {
              throw new HttpException('Bad Request', 400);
  ...
  async joinStakePool(stakePoolId: string, walletId: string, input: PutStakePoolsStakePoolIdWalletsWalletIdInput): Promise<any> {
        ...
        try {
          const response = await this.apiClient.stakePools.joinStakePool({ stakePoolId, walletId }, input as JoinStakePoolPayload, extraConfig);
          return response.data;
        } catch (error: any) {
            if (error.response?.status === 400) {
              throw new HttpException('Bad Request', 400);
