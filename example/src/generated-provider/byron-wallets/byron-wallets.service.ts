...
export class ByronWalletsService {
  ...
  async postByronWallet(input: PostByronWalletsInput): Promise<any> {
        ...
        try {
          const response = await this.apiClient.byronWallets.postByronWallet(input as PostByronWalletPayload, extraConfig);
          return response.data;
        } catch (error: any) {
  ...
  async putByronWallet(walletId: string, input: PutByronWalletsWalletIdInput): Promise<any> {
        ...
        try {
          const response = await this.apiClient.byronWallets.putByronWallet({ walletId }, input as PutByronWalletPayload, extraConfig);
          return response.data;
        } catch (error: any) {
            if (error.response?.status === 400) {
              throw new HttpException('Bad Request', 400);
  ...
  async putByronWalletPassphrase(walletId: string, input: PutByronWalletsWalletIdPassphraseInput): Promise<any> {
        ...
        try {
          const response = await this.apiClient.byronWallets.putByronWalletPassphrase({ walletId }, input as PutByronWalletPassphrasePayload, extraConfig);
          return response.data;
        } catch (error: any) {
            if (error.response?.status === 400) {
              throw new HttpException('Bad Request', 400);
